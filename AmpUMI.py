...
def dedupUMIs(args,parser):
    ...
    barcode_keys_with_most_counts = {} #barcode->key (key has most counts)
    barcode_key_counts = {} #barcode->count (count of fastqs key has seen) 

    barcode_seq_counts = {} #barcode_seq->count of that pair
    barcode_seq_best_qual_fastqs = {} #barcode_seq->fastq to be printed
    barcode_seq_best_qual_sum = {} #best qual sum for the best fastq
    with open (args.fastq,"r") as f_in:
        ...
        if match_obj:
            ...
            if this_key not in barcode_seq_counts:
                barcode_seq_counts[this_key] = 1
                barcode_seq_best_qual_sum[this_key] = trimmed_qual_sum
                barcode_seq_best_qual_fastqs[this_key] = id_line + trimmed_seq + plus_line + trimmed_qual
            else:
                barcode_seq_counts[this_key]['count'] += 1
                if barcode_seq_best_qual_sum[this_key] < trimmed_qual_sum:
                    barcode_seq_best_qual_sum[this_key] = trimmed_qual_sum
                    barcode_seq_best_qual_fastqs[this_key] = id_line + trimmed_seq + plus_line + trimmed_qual

            if this_UMI not in barcode_key_counts:
                barcode_key_counts[this_UMI] = 1
                barcode_keys_with_most_counts[this_UMI] = this_key 
            else:
                if barcode_seq_counts[this_key] > barcode_key_counts[this_UMI]:
                    barcode_key_counts[this_UMI] =  barcode_key_counts[this_UMI]
                    barcode_keys_with_most_counts[this_UMI] = this_key

    if read_count == 0:
        raise UMIException("UMI parsing command failed. Got no reads with regex.")
    ...
    for barcode_seq in barcode_seq_counts:
        barcode, seq = barcode_seq.split(" # ")
        #if another barcode had more reads than ths one...
        if barcode_keys_with_most_counts[barcode] != barcode_seq:
            collision_count += 1
            collision_count_reads += barcode_seq_counts[barcode_seq]
        #if this umi had too few reads
        elif barcode_seq_counts[barcode_seq] < args.min_UMI_to_print:
            too_few_reads_count += 1
            too_few_reads_count_reads += barcode_seq_counts[barcode_seq]
        else:
            f_out.write(barcode_seq_best_qual_fastqs[barcode_seq])
            printed_count += 1
            printed_count_reads += barcode_seq_counts[barcode_seq]

    f_out.close()

    print("Observed %d UMI collisions (%d reads)"%(collision_count,collision_count_reads))
    print("Didn't print sequences for %d UMIs because they had too few reads (%d reads)"%(too_few_reads_count,too_few_reads_count_reads))
    print("Printed %d deduplicated sequences (%d reads) to %s"%(printed_count,printed_count_reads,args.fastq_out))
    

def calculateUMIsmath(Q,n):
        try:
                xrange
        except NameError:
                xrange = range
...
def calculateUMIs(args,parser):
            ...
            barcodeLength = 0
            if args.nb:
                barcodeCount = args.nb
                barcodeLength = mpmath.log(barcodeCount,4)
                Q = mpf(mpf(args.nb)*mpf(args.na))
            elif args.bl:
                Q = mpf(4**args.bl)*mpf(args.na)
            else:
                parser.print_help()
                exit("Either -bl or -nb is required for barcode length calculation")
            n = args.n
            p = calculateUMIsmath(Q,n)
            print("With %d barcodes (length %d) and %d unique allels,  in %d reads, the probability of no collisions is %f"%(barcodeCount,barcodeLength,args.na,args.n,p))
...
def main():
        ...
        parser_run.add_argument('--umi_regex',help='Regular expression specifying the barcode (I) as well as any primer sequences to be trimmed (A,C,T,G)\nFor example, if the UMI is the first 5 basepairs, this should be "^IIIII".',required=True)
        ...
        parser_calculate.add_argument("-n",type=int,help="Number of molecule-UMI pairs sequenced")
        parser_calculate.add_argument("-bl",type=int,help="Barcode length",required=false)
        parser_calculate.add_argument("-nb",type=int,help="Number of unique barcodes",required=false)
